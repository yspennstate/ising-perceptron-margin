...
at the margin $\kappa=0.05$ by porting the zero-margin certificate
...
Mills-ratio bounds.  At that margin all four hypotheses hold, so
Huang's theorem gives an unconditional capacity upper bound
$\alpha_\star(0.05)\in[0.781073068,0.781074776]$.  The verification
...
The storage capacity of the Ising perceptron at margin $\kappa=0.05$
is at most $\alpha_\star(0.05)\in[0.781073068,\,0.781074776]$: for
$\alpha$ above this interval, with probability $1-o(1)$ no
$w\in\{-1,+1\}^N$ satisfies all $\alpha N$ constraints.
\end{theorem}

The same pipeline is parameterized over the located boxes of any
certified slab, and nothing in it is specific to the point
$\kappa=0.05$ beyond compute time; extending
Theorem~\ref{thm:main} to the remaining certified margins and then
to $\kappa$ sub-slabs of the strip is mechanical and underway.  On
the strip minus that extension, the precise statement is:
Conditions~\ref{cond:fp}--\ref{cond:lc} hold for every
$\kappa\in[-0.65,0.19]$, and Condition~\ref{cond:2var} is certified
at $\kappa=0.05$ and at $\kappa=0$ (where our pipeline independently
reproduces the zero-margin verification).
