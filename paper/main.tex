...
$G$ evaluator depth $16000$).  The wide-rectangle product $1.32$ was
a bound artifact, not a property of the map: on a thin located
rectangle the certified contraction clears $1$ at $\kappa=0.13$ and
on the sampled slabs at the lane's lower edge (per $\kappa$
sub-ball, matching the lane's own subdivision;
\texttt{rect\_strip\_nak.jsonl}), so the same driver discharges the
rectangle clause slab by slab where run.  We certify it in full only
at $\kappa=0.13$; extending it across the lane is compute, not new
mathematics.
