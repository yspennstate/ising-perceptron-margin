...
clears $1$.  We certify the clause in full only at $\kappa=0.13$;
extending it across the lane is that per-slab deep location plus a
thin rectangle --- compute, not new mathematics, since the
...
For every $\kappa\in[-0.65,0.19]$,
Conditions~\ref{cond:fp}--\ref{cond:lc} hold: $\alpha_\star(\kappa)$
exists, is unique on the fixed-point branch, and lies in the
certified $\alpha$ interval of the slab containing $\kappa$.
Consequently, at every $\kappa$ in the strip at which
Condition~\ref{cond:2var} holds, and for every $\alpha$ above that
slab interval's upper endpoint, with probability $1-o(1)$ no
...
$(0.10,0.19]$, and the rectangle clause again at $\kappa=0.13$
