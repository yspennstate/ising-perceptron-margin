...
exactly at the fixed point, so on each certified branch rectangle
the threshold $\alpha_\star(\kappa)$ is the unique root of a strictly
decreasing function; and $\partial R/\partial q$ collapses to a
one-term integral with positive integrand, a form of the
general-margin monotonicity of Ding and Sun that interval arithmetic
can consume directly.  Building on these we verify, in ball arithmetic over
...
carrying per-slab thin-rectangle contractions and, independently,
Nakajima's saddle theorem under a certified premise), with
...
\url{https://github.com/yspennstate/ising-perceptron-margin}.
  On the lower-bound side, open at every nonzero
margin, we prove that unconditioned second moments fail at every
margin (with a closure over nonnegative weights for
$\kappa\ge0$), derive capacity lower bounds for all $\kappa<0$ by
embedding a symmetric perceptron, and verify in floating point that
the general-margin form of Ding--Sun's lower-bound condition holds
at all five certified margins.
\end{abstract}
...
The general-$\kappa$ curve has been predicted before by non-rigorous
means --- Stojnic's fully-lifted random duality
theory~\cite{stojnic2023} reproduces the same replica-symmetric
values across margins --- so what is new here is not the numerical
value but its rigorous certification, which had stopped at
$\kappa=0$.
...
\kappa)$; by the envelope theorem, along a differentiable fixed-point
branch, $G_\star'(\alpha)=\E\log\Psi(u)<0$ strictly, so on every
certified rectangle --- where the contraction makes the branch exist,
stay unique, and vary smoothly --- $\alpha_\star(\kappa)$ is the
unique root of a strictly decreasing function.  And the derivative $\partial R/\partial
...
The certified theorems here are upper bounds.  The matching lower
bound at $\kappa\ne0$ remains open --- the embedded bound of
Section~\ref{sec:lower} is rigorous for every $\kappa<0$ but far
from matching --- so no statement in this paper
...
function; $M'=M\,(M-u)\in(0,1)$, proved with its second-derivative
bounds in Lemma~\ref{lem:mills}), and $Z\sim N(0,1)$.  Fix
...
point for every $\alpha$ in a stated interval, with
$\sup_{q\in(q_{\mathrm{lb}},q_{\mathrm{ub}})}(P\circ R_\alpha)'(q)<1$
uniformly on the rectangle, and $G_\star$ crosses zero inside that
interval.  The rectangle is existentially quantified: Huang's own
$\kappa=0$ instantiation (his Proposition~3.2) takes an interval of
width $10^{-9}$ about the fixed point.
\end{cond}

\begin{cond}[AMP]\label{cond:amp}
$\alpha\,\E[\sech^4(\sqrt{\psi_0}Z)]\;
\E[M'(u(Z;q_0,\kappa))^2]/(1-q_0)^2\;<\;1$ at the fixed point,
with $u(z;q,\kappa)=(\kappa-\sqrt q\,z)/\sqrt{1-q}$ as above.
\end{cond}
...
Both derivatives vanish exactly on the fixed-point branch.  On a
rectangle carrying the contraction of Condition~\ref{cond:fp} the
branch $\alpha\mapsto(q_\star,\psi_\star)$ is $C^1$ (the implicit
function theorem applies: $1-(P\circ R_\alpha)'$ is bounded away
from zero), and the envelope theorem gives
$G_\star'(\alpha)=\E\log\Psi(u)<0$ strictly ($\Psi<1$ pointwise), for
every $\kappa$: the root $\alpha_\star(\kappa)$ is unique on the
branch.  This is the
...
\]
and the $-\alpha\E[M^2]/(1-q)^2$ piece cancels $R/(1-q)$ exactly.
Positivity is immediate since $M'\in(0,1)$.  In both proofs
differentiation under the integral sign is dominated
($|z\tanh(\sqrt\psi z)|\le\sqrt\psi\,z^2$, and the $q$-integrands
grow at most quadratically in $|z|$, locally uniformly in the
parameters), and Stein's lemma applies because $M(u(z))$ and
$(MM')(u(z))$ are absolutely continuous with bounded derivative in
$z$.
\end{proof}
...
the Gardner spherical capacity~\cite{gardner1988}, and Nakajima's saddle-existence bound
...
at $\kappa=1.5$).  Toward $\kappa=-2$, the lower edge of the computed range, the
threshold approaches the annealed bound from below, as it must where the constraints
...
checked, confirming numerically Huang's identity for the tilt (stated
by him at general margin) along the curve --- this observation later
becomes exact and load-bearing (Section~\ref{sec:regionI}).
...
(the $q$ pad must cover the fixed point's drift across the slab's
$(\alpha,\kappa)$ rectangle, dominated by $\partial q_\star/\partial
\alpha\approx0.92$ times the $\alpha$ pad; the direct drift
$dq_\star/d\kappa$ stays near $0.18$ across the strip), and one on the
...
bisection of the displacement $P(R(q))-q$ --- which has certified signs at the interval endpoints and, by
the uniqueness of the fixed point, a single zero between them, so it
is positive below the fixed point and negative above --- locates the fixed point at both $\alpha$
...
The rectangle clause itself is then discharged at $\kappa=0.13$
exactly as Huang's Proposition~3.2 discharges it at $\kappa=0$: on
the rectangle $\alpha\in[0.705932217,\,0.705933898]$,
$q\in[0.5836943,\,0.5857104]$ (the located fixed points
$\pm10^{-3}$), the certified contraction product is $0.8435$, the
map is endpoint-inward at both $\alpha$ ends with margins above
$2.3\cdot10^{-4}$, the $G$ signs at the located fixed points are
$+1.6\cdot10^{-7}$ and $-1.6\cdot10^{-6}$, and $\E\log\Psi<0$
over the rectangle (\texttt{rect\_cond31\_0p13.json}; locate and
$G$ evaluator depth $16000$).  The wide-rectangle product $1.32$ is a
pad artifact, not a property of the map, and the map itself carries
no wall across the lane: the true fixed-point contraction
$(P\circ R_\alpha)'(q_\star)$ is nearly flat, rising only from
$0.758$ at $\kappa=0.10$ to $0.770$ at $\kappa=0.19$, everywhere
below $1$ with margin above $0.22$.  What decides the rectangle
clause is therefore the pad width, not the margin: a probe over the
lane (\texttt{rect\_strip\_nak.jsonl}, per $\kappa$ sub-ball) clears
$1$ at the lower edge with a $2\cdot10^{-3}$ half-pad but crosses it
($1.012$) by $\kappa=0.15$ as that same pad inflates the flat
$0.765$, exactly as a thinner located rectangle pulls $\kappa=0.13$
back to $0.844$.  The pad that matters is the located $q$ interval,
whose width is set mostly by the slab's $\alpha$ width
($3\cdot10^{-3}$) through $\partial q_\star/\partial\alpha\approx0.9$;
narrowing it means deep-locating $\alpha_\star$ per slab to
$\sim10^{-6}$, the same tightening the five point margins already
carry, after which the flat contraction leaves a thin rectangle that
clears $1$.  We first ran this recipe at both ends of the lane and an interior
point ($\kappa\in\{0.13,0.15,0.185\}$; contractions $0.844$, $0.836$,
$0.830$ on deep-located rectangles;
\texttt{rect\_cond31\_0p15.json}, \texttt{rect\_cond31\_0p185.json}),
and then per slab across the whole lane: on each of the $180$ slabs,
over eight $\kappa$ sub-balls of width $6.25\cdot10^{-5}$, the
located rectangle with half-pad $5\cdot10^{-4}$ is endpoint-inward at
both $\alpha$ ends and carries a certified contraction product below
one, the worst bound across the lane being $0.958$
(\texttt{rect\_strip\_nak\_thin\_*.jsonl}, collected with the input
hashes bound in \texttt{rect\_strip\_thin\_summary.json}).  With the
inward endpoints and Lemma~\ref{lem:dR}'s monotonicity this gives,
for every $\kappa$ in the lane, a rectangle on which the fixed point
is unique by the contraction itself, so the rectangle clause of
Condition~\ref{cond:fp} holds on every slab with uniqueness carried
both by the rectangle and, independently, by Nakajima's theorem.
...
general-margin $C_j$ directly at the shifted law.  With
$\hat H=\sqrt{q_0}\,Z$, $u=(\kappa-\hat H)/\sqrt{1-q_0}$,
$N=M(u)/\sqrt{1-q_0}$, $F'=-M'(u)/(1-q_0)$, and
$Q=-\hat H/q_0+\bigl(\kappa-\hat H-(1-q_0)N\bigr)/(1-q_0)$,
every expectation over the single Gaussian $\hat H$,
\[
C_1=\frac{\alpha}{\psi_0^2}\,\E[F'N^2],\qquad
C_2=-\frac{2\alpha}{\psi_0}\,\E[F'NQ]+\frac{2}{1-q_0},
\]
\[
C_3=\alpha\,\E[F'Q^2]+\frac{2\alpha}{q_0(1-q_0)}\,\E[N\hat H]
-\alpha\Bigl(\frac{3}{(1-q_0)^2}+\frac{1}{q_0(1-q_0)}\Bigr)
\E\bigl[N\,(\kappa-\hat H-(1-q_0)N)\bigr].
\]
The certified evaluation encloses each expectation by the mean-value
cell rule with explicit quadratic tail majorants ($N$ grows at most
linearly on the left tail, $F'$ is bounded by $1/(1-q_0)$ in
magnitude, and every integrand is degree at most two in $|z|$), and
the near-cancelling prior moments are evaluated as single correlated
integrands.  The certified enclosures at the six margins are
one-sided where sign is the claim:
\[
\begin{array}{lll}
\kappa=-0.45: & M_{11}\le-0.0647, & \det M\ge 2.9\cdot10^{-4},\\
\kappa=-0.05: & M_{11}\le-0.0471, & \det M\ge 1.6\cdot10^{-4},\\
\kappa=0:     & M_{11}\le-0.0451, & \det M\ge 1.8\cdot10^{-4},\\
\kappa=0.05:  & M_{11}\le-0.0424, & \det M\ge 1.2\cdot10^{-4},\\
\kappa=0.0995:& M_{11}\le-0.0400, & \det M\ge 1.0\cdot10^{-4},\\
\kappa=0.13:  & M_{11}\le-0.0385, & \det M\ge 8.8\cdot10^{-5},
\end{array}
\]
the trend --- both margins thinning as $\kappa$ grows --- being the
quantitative form of the landscape flattening that
Section~\ref{sec:interval} prices.
...
at the fixed point, so the quadratic tilt coefficient cancels, and
wherever the surviving linear coefficient of the tilt is nonpositive
the functional runs away (the measured boundary of that region at
$\kappa=0.05$ crosses $\lambda_1=0.04$ on the $\lambda_2=0$ axis and
bends left as $|\lambda_2|$ grows).  Nothing downstream consumes
this observation: the condition asks for the uncapped infimum over
$s\ge0$, so a certified negative value at one explicit finite tilt
--- which is what the sweep certificates exhibit cell by cell --- is
already a complete witness, and the runaway directions are the
capped-tilt case handled inside Huang's own argument.
...
derivative is $-6.6$).  Three elementary bounds, applied as
intersections with the computed balls, remove the failure mode
entirely.

\begin{lemma}[Mills clamps]\label{lem:mills}
Let $E=\varphi/\Psi$ be the Gaussian hazard and $t=E-V$.  For every
$V\in\R$,
\[
t>0\ \ \Bigl(\text{and } t<\frac1V \text{ for } V>0\Bigr),\qquad
E'=Et\in(0,1),\qquad
0<E''<t .
\]
\end{lemma}
...
\begin{proof}
For $t>0$: when $V\le0$ this follows from $E>0$, and when $V>0$ integrating
$\varphi(z)\le(z/V)\varphi(z)$ over $z\ge V$ gives
$\Psi(V)<\varphi(V)/V$.  For $t<1/V$: the function
$r(V)=\Psi(V)-V\varphi(V)/(V^2+1)$ has
$r'(V)=-2\varphi(V)/(V^2+1)^2<0$ and vanishes at $+\infty$, so $r>0$,
which rearranges to $E<V+1/V$.  Next, $E'=E^2-VE=Et>0$, while the
conditional moments $\E[Z\mid Z>V]=E$ and $\E[Z^2\mid Z>V]=1+VE$ give
$1-E'=\operatorname{Var}(Z\mid Z>V)>0$.  The upper bound on $E''$ is
the identity $E''-t=(E'-1)(E+t)$ with $E'<1$ and $E+t>0$.

Positivity of $E''$ --- the convexity of the Gaussian hazard --- is
the one clause that takes an argument.  Differentiating $E'$ gives
$E''=E\,[\,t(E+t)-1\,]$, and for $W$ distributed as $Z-V$ conditioned
on $\{Z>V\}$ the moment formulas above read $\E W=t$ and
$\E[W^2]=1-VE+V^2$, whence $t(E+t)-1=2(\E W)^2-\E[W^2]$: the claim is
that this residual law has coefficient of variation below one.  Its
survival function is
$\Prob(W>w)=\Psi(V+w)/\Psi(V)=e^{-H(w)}$ with
$H(w)=\int_0^w E(V+x)\,dx$ convex, since $E'>0$.  Set $\theta=1/t$,
so the rate-$\theta$ exponential has the same mean, and
$J(w)=H(w)-\theta w$.  Then $J$ is convex with $J(0)=0$,
$J'(0)=E-1/t<0$ (this is $Et=E'<1$ again), and $J(w)\to\infty$, so
$J<0$ exactly on some interval $(0,w_0)$ and $J>0$ past it.  Hence
$\Prob(W>w)-e^{-\theta w}=e^{-\theta w}(e^{-J(w)}-1)$ is positive
before $w_0$ and negative after, while
$\int_0^\infty(\Prob(W>w)-e^{-\theta w})\,dw=t-t=0$.  Therefore
\[
\tfrac12\,\E[W^2]-t^2
=\int_0^\infty w\,\bigl(\Prob(W>w)-e^{-\theta w}\bigr)\,dw
=\int_0^\infty (w-w_0)\bigl(\Prob(W>w)-e^{-\theta w}\bigr)\,dw<0,
\]
the last integrand being nonpositive and not almost everywhere zero.
So $\E[W^2]<2(\E W)^2$, that is $E''>0$, at every real $V$.  (This
specializes the classical fact that increasing-hazard laws have
coefficient of variation at most one; strictness here is the strict
monotonicity of the hazard.)  The algebraic steps --- the $E''$
formula, the equivalence, and the moment identities --- are verified
symbolically (\texttt{sympy\_identities.py}).
\end{proof}

Intersecting a computed
...
$1365$ leaves, zero failures, with star containment read from the
...
certified slab; what stands between the point theorem and a
$\kappa$-interval version of Condition~\ref{cond:2var} is quantified
in Section~\ref{sec:interval}.  The other three conditions do not
wait for that extension: they are certified over the whole strip,
which gives the conditional structure of the program a closed form.

\begin{theorem}\label{thm:strip}
For every $\kappa\in[-0.65,0.10]$, Conditions~\ref{cond:fp}--%
\ref{cond:lc} hold, with the uniform rectangle contraction of
Condition~\ref{cond:fp} certified on the slab rectangle itself.  For
every $\kappa\in(0.10,0.19]$, Condition~\ref{cond:fp} holds per slab
in full --- existence, crossing, and the rectangle contraction on a
thin located rectangle over $\kappa$ sub-balls, with uniqueness from
the rectangle contraction and, independently, from Nakajima's
theorem under a certified premise --- and Conditions~\ref{cond:amp}
and~\ref{cond:lc} hold per slab.  In every case
$\alpha_\star(\kappa)$ exists, is unique on the fixed-point branch,
and lies in the certified $\alpha$ interval of the slab containing
$\kappa$.  Consequently, at every $\kappa$ at which
Condition~\ref{cond:2var} and the full Condition~\ref{cond:fp} both
hold --- in particular the five margins of
Theorem~\ref{thm:main} --- and for every $\alpha$ above that slab
interval's upper endpoint, with probability $1-o(1)$ no
$w\in\{-1,+1\}^N$ satisfies all $\alpha N$ constraints at margin
$\kappa$.
\end{theorem}

Condition~\ref{cond:2var} is certified at the five margins of
Theorem~\ref{thm:main} and at $\kappa=0$, where our pipeline
independently reproduces the zero-margin verification; at those six
values the hypothesis of Theorem~\ref{thm:strip} discharges and the
two theorems agree.  Everywhere else on the strip, the entire
distance between the conditional and unconditional statements is one
two-variable inequality at that single margin.

\input{margins_appendix}

\input{lower_bound_section}
...
\emph{Exact artifacts and replay.}  The two-variable chain's
certificates carry every
sign-critical quantity as outward-rounded decimal packets in
canonical JSON, with SHA-256 source and payload binding and atomic
no-clobber publication; the stage-2 artifact binds the hash of the
Region-I manifest it consumed.  (Some schedule-geometry fields
remain ordinary JSON floats --- they parameterize display and tiling
bookkeeping, not the inequalities --- and the strip-slab and
rectangle files, which predate the packet schema, record their
enclosures as display strings; their sign claims enter the kernel
layer only as those manifests are regenerated.)  A
...
outward in exact rationals, and the tilings within each serialized
chain.  It also binds each recorded leaf value to the inequality it
stands for --- every cell or band marked passing must carry a
nonpositive worst-case $\sS_\star$ and no subcell failure --- so a
flipped flag or a mismarked positive value is caught rather than
trusted (the transcendental re-evaluation itself remains the
ball-arithmetic layer's job).  The checker also enforces two global
completeness clauses on
the band geometry: the innermost ring must close the full circle as
a single seam-exact run, and each level's angular support must sit
inside the previous level's, which together make every direction's
radial chain contiguous out to the radius where its support ends ---
a deliberately corrupted manifest with all hashes consistently
rebound fails exactly this check.  The outermost band level is bound
to the star radius recorded in the same manifest, which is the reach
the stage-2 containment consumes and binds by hash.  The Lean
tilings carry per-chain scope only; that remains recorded as open
hardening.
...
Lean~4, following the zero-margin release: per-margin generated files
whose kernel-decided theorems cover the $1404$-band radius
chain and its angular runs, both sweep grid tilings, and the
star-interior packet chain, with the Loewner-majorant family at all
five margins (the $\kappa=0.05$ manifests, which predated the
$B$-packet schema, were regenerated through the full pipeline).  The
extraction is mirrored in exact
...
integer per file) rejects every mutant.  The slab-level sign claims
...
on $\kappa\in[-0.65,0.10]$, and the same on
$(0.10,0.19]$ with the rectangle clause certified per slab on thin
located rectangles
(Theorem~\ref{thm:strip}); deep
$\alpha_\star$ intervals at five margins; the center Hessian at six;
Condition~\ref{cond:2var} at
$\kappa\in\{-0.45,-0.05,0,0.05,0.0995,0.13\}$; and hence Huang's
unconditional upper bound at the five nonzero margins
(Theorem~\ref{thm:main}), with the rest of the strip conditional
only on Condition~\ref{cond:2var} at the margin in question.  The matching lower
bound is open: Ding--Sun's conditioned second-moment reduction
is proved at $\kappa=0$ only, and the embedded bound of
Section~\ref{sec:lower}, while rigorous at every $\kappa<0$, sits
far below the replica-symmetric value; nothing here is an exact
threshold statement at $\kappa\ne0$.  All float scans are diagnostics, not
...
arXiv:2607.11010 (2026); verification repository at
...
\bibitem{perkinsxu2021}
W.~Perkins and C.~Xu,
Frozen 1-RSB structure of the symmetric Ising perceptron,
STOC 2021; arXiv:2102.05163.

\bibitem{abbelisly2021}
E.~Abbe, S.~Li and A.~Sly,
Proof of the contiguity conjecture and lognormal limit for the
symmetric perceptron,
FOCS 2021; arXiv:2102.13069.

\bibitem{altschuler2022}
D.~J.~Altschuler,
...
Sharp threshold sequence and universality for Ising perceptron
models,
arXiv:2204.03469.
